"""CVAP demographic data aggregated to precinct level using spatial intersection."""
...
from ..sources import CVAPBlockGroupNode, AlabamaBlockGroupsNode, AlabamaVotingDistrictsNode


@register_node
class CVAPPrecinctNode(DataNode):
    """Aggregate CVAP block group demographic data to voting precinct level.
    ...
    Output columns:
    - precinct_id: Unique precinct identifier (VTD GEOID)
    ...
    dependencies = [CVAPBlockGroupNode, AlabamaBlockGroupsNode, AlabamaVotingDistrictsNode]
    ...
    def process(self, inputs: Dict[Type[DataNode], pd.DataFrame]) -> pd.DataFrame:
        cvap_df = inputs[CVAPBlockGroupNode]
        block_groups_gdf = inputs[AlabamaBlockGroupsNode]
        voting_districts_gdf = inputs[AlabamaVotingDistrictsNode]

        # Convert to equal-area projection for accurate area calculations (Alabama State Plane)
        equal_area_crs = 'EPSG:26930'  # NAD83 / Alabama West

"""CVAP block group-level data loader."""
...
@register_node
class CVAPBlockGroupNode(DataNode):
    """Load block group-level CVAP data for Alabama.

    Loads demographic data from BlockGr.csv filtered for Alabama (FIPS 01).
    ...
    def process(self, inputs: Dict[Type[DataNode], pd.DataFrame]) -> pd.DataFrame:
        ...
        # Filter for Alabama (FIPS 01)
        df = df[df['state_fips'] == '01'].copy()
